...
- Next, start in the [examples/tests](examples/tests) directory which has very simple examples that demonstrate the interesting features of tensor trains, but also serve as our CI testing suite.
- Then, move on to our [examples/exercises](examples/exercises), which demonstrate some use cases of tensor decompositions using BoBa.
...
If you are reproducing a CI-style run, add `--ci`.
...
- When building, add the flag `BOBA_DEBUG=1` to (for example) perform bounds checking on all array accesses (except inside GPU kernels) and generate other debugging information
```
